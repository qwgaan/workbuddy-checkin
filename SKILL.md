...
> **本仓库为 Windows 兼容独立分支**：基于 [`cat-xierluo/legal-skills`](https://github.com/cat-xierluo/legal-skills) 的 `workbuddy-checkin` v1.0.2，合入 3 个 Windows / PowerShell 5.1 修复（见 `README.md` 与 `CHANGELOG.md`「本地补丁记录」）。**macOS / Linux 用户建议直接用上游原版**。
...
### 在 WorkBuddy 内（Agent 自动化，推荐）

WorkBuddy 环境下用 `automation_update` 工具（recurring 类型）创建定时任务即可，**无需 crontab / 任务计划程序 / launchd**。下面配置已在本机实测可用：
...
```jsonc
{
  ...
  "status": "ACTIVE",
  "prompt": "执行 Buddy 加油站每日签到。保持极简——不要创建任务列表、不要使用任务管理工具、不要把结果拆成多条消息。1) 设置运行时 $env:WB_CHECKIN_NODE='C:\\Users\\admin\\.workbuddy\\binaries\\node\\versions\\22.22.2\\node.exe'；2) 运行 Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force; & \"C:\\Users\\admin\\.workbuddy\\skills\\workbuddy-checkin\\scripts\\checkin.ps1\"；3) 读取 logs/checkin.log 末行判定结果（签到成功 / 今日已签到 / 失败）；4) 把当天一行追加进 签到历史.md（markdown 表格，最末行即最近结果）；5) 可见回复只输出一句话，不回显任何 token 或凭据。"
}
```

> 说明：`cwds` 指向一个**专门的空目录**（如 `定时任务\\workbuddy签到`），自动化运行记录会在「空间」侧栏里折叠显示、不污染主工作区；`签到历史.md` 把所有每日结果汇聚到同一文件，避免每天生成一堆任务列表。注意：`update` 已有任务时必须显式传 `rrule`，否则可能被重置；`cwds` 不能用 Claw 工作区。上述 Node / 技能路径为本机实测值，其他机器请按实际 `~/.workbuddy/binaries/node/versions/<版本>/node.exe` 与技能目录调整。

也可以直接对 WorkBuddy 说：「创建一个每天 9 点自动运行 workbuddy-checkin 签到的自动化」，让它按上述参数创建。
