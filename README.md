...
> # ⚠️ 本仓库已废弃（DEPRECATED）
>
> 本仓库的方案**已失效**：WorkBuddy 客户端后来把登录令牌改为非对称加密信封，
> 并强制要求 `X-Device-Token` 风控头，原「读本地令牌 + 直连签到接口」的路线不再可用。
>
> ## 👉 请改用新方案：[qwgaan/workbuddy-checkin-cdp](https://github.com/qwgaan/workbuddy-checkin-cdp)
>
> 新方案改为通过 CDP 在渲染进程内调用客户端自身的内部 RPC，**不读取、不解密任何令牌**，
> 鉴权完全由客户端自己完成，因此不受令牌加密变更影响。
>
> 本仓库仅作历史参考保留，**不再维护、不再更新**。
>
> ---
>
> _废弃日期：2026-09-25_
